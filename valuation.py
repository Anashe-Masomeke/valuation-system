...
hist_year_ints = [int(str(y)) for y in year_cols_is]
...
# ---------------------------------------------------------
items_is_lower = is_df["Item"].astype(str).str.lower()
...
all_cols_for_totals = list(year_cols_is) + list(forecast_cols)
...
# Extract EBITDA & Depreciation for forecast years
...
if dep_hist_from_is_idx is not None:
    ...
    # If IS depreciation row is missing, use CF-based ratio (if mapped)
    if dep_cf_idx_list:
        dep_hist_vals = cf_df.loc[dep_cf_idx_list, year_cols_cf].sum(axis=0).values.astype(float)
...
# Working Capital from CA - CL for each year
wc_ratio = 0.0
...
if ca_idx_list and cl_idx_list:
    ...
    common_years_for_wc = [c for c in year_cols_is if c in wc_hist.index]
    if common_years_for_wc:
        wc_vals = wc_hist[common_years_for_wc].values.astype(float)
        rev_vals_wc = revenue_row[common_years_for_wc].values.flatten().astype(float)
        wc_ratio = ratio_to_revenue(wc_vals, rev_vals_wc)

        # baseline WC at last historical year
        if bs_year_used_label in wc_hist.index:
            wc_prev = float(wc_hist[bs_year_used_label])
        else:
            wc_prev = float(revenue_row[last_hist_label]) * wc_ratio

        deltas = []
        for y in forecast_years_int:
            wc_curr = rev_forecast[y] * wc_ratio
            deltas.append(wc_curr - wc_prev)
            wc_prev = wc_curr
        delta_wc_forecast_vals = np.array(deltas, dtype=float)
...
if capex_cf_idx_list:
    ...
    if common_capex:
        ...
        rev_vals_capex = revenue_row[common_capex].values.flatten().astype(float)
        capex_ratio = ratio_to_revenue(capex_hist_vals, rev_vals_capex)

capex_forecast_vals = np.array(
    [rev_forecast[y] * capex_ratio for y in forecast_years_int],
    dtype=float
)
...
# ---------------------------------------------------------
# FCFF = EBITDA(1−T) + Dep·T − ΔWC − Capex
# ---------------------------------------------------------
fcff_vals = (
    ebitda_forecast_vals * (1 - tax)
    + dep_forecast_vals * tax
    - delta_wc_forecast_vals
    - capex_forecast_vals
)

# ---------------------------------------------------------
# DCF: DISCOUNT FCFF + TERMINAL
# ---------------------------------------------------------
n = len(forecast_years_int)
discount_factors = np.array([(1 / (1 + wacc) ** i) for i in range(1, n + 1)])
pv_fcff = fcff_vals * discount_factors

fcff_last = fcff_vals[-1] * (1 + g)
if wacc <= g:
    terminal_value = np.nan
    pv_terminal = np.nan
else:
    terminal_value = fcff_last / (wacc - g)
...
# DCF TABLE
# ---------------------------------------------------------
st.subheader("📉 DCF Cashflows (UFCF)")

df_dcf = pd.DataFrame(
    {
        "Year": forecast_years_int,
        "Revenue": [rev_forecast[y] for y in forecast_years_int],
        "EBITDA": ebitda_forecast_vals,
        "Depreciation": dep_forecast_vals,
...
fmt_dict = {c: "{:,.0f}".format for c in num_cols_dcf if c != "Discount factor"}
styled_dcf = df_dcf.style.format(fmt_dict, na_rep="").format({"Discount factor": "{:.3f}".format})
...
st.dataframe(
    df_term.style.format(
        {c: "{:,.0f}".format for c in df_term.select_dtypes(include=[np.number]).columns},
        na_rep="",
    ),
    use_container_width=True,
)
